...
export function HabitList({ habits, groups, selectedDate, onHabitCompletion }: HabitListProps) {
  ...
  if (habits.length === 0) {
    return (
        <div className="text-center py-16">
            <p className="text-lg font-medium text-muted-foreground">No hay hábitos para este día.</p>
            <p className="text-sm text-muted-foreground">¡Añade uno nuevo para empezar!</p>
        </div>
    )
  }
      ...
      {groupOrder.map(groupId => {
        ...
        return (
          <div key={groupId} className="space-y-4">
            {groupInfo && <h2 className="text-2xl font-bold text-foreground/80">{groupInfo.name}</h2>}
